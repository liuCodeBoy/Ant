...
import UIKit
import MJRefresh
class InviteJobVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    
     let houseRentCell = "LunTanListWithLocationCell"
    ...
    override func viewDidLoad() {
        ...
        //注册cell
        tableView?.register(UINib.init(nibName: "LunTanListWithLocationCell", bundle: nil), forCellReuseIdentifier: houseRentCell)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        ...
        let  cell = tableView.dequeueReusableCell(withIdentifier: houseRentCell) as? LunTanListWithLocationCell
        //把模型的数据传给cell
    ...
    func  tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let recruitDVC =  RecruitDVC()
        //取出模型
        let  model = self.modelInfo[indexPath.row]
        recruitDVC.RecruitID = Int(model.id!)
        recruitDVC.urls = model.picture!
        self.navigationController?.pushViewController(recruitDVC, animated: true)
    }
...
extension InviteJobVC {
    ...
    fileprivate func loadCellData(page : Int) {
        ...
        //将当前的下载操作添加到组中
        group.enter()
        NetWorkTool.shareInstance.infoList(VCType: .job, cate_2 : "", cate_3 : "", cate_4 : "2", p: page) { [weak self](result, error)  in
            //在这里异步加载任务
