...
extension HouseRentListVC: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        let  cell = tableView.dequeueReusableCell(withIdentifier: houseRentCell) as! HouseRentViewCell
        //把模型的数据传给cell
        cell.viewModel = modelInfo[indexPath.row]
        
        if indexPath.row == 0 {
            cell.topLabel.isHidden = false
            cell.title.text = "            \(String(describing: (cell.viewModel?.title)!))"
        } else {
...
// MARK:- Load viewController basic info
